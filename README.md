...
![ModelPort overview](docs/assets/modelport-overview.svg)
...
![ModelPort quick start](docs/assets/modelport-quickstart.svg)
...
It exposes an Anthropic-compatible `/v1/messages` endpoint on your machine and routes requests to Mimo, DeepSeek, Anthropic, OpenAI-compatible providers, OpenRouter, Ollama, or a custom upstream.

### Status

ModelPort is production-ready within its intended scope:

- Personal long-running local usage.
- Persistent local gateway for VS Code Claude / Claude Code.
- Small internal team production or pilot deployments behind a trusted network boundary.

It is not designed to be directly exposed to the public internet, and it is not a multi-tenant SaaS gateway with billing, quotas, user accounts, audit retention, or RBAC.
...
- Anthropic-compatible API entry point.
- OpenAI-compatible upstream conversion.
- `provider:model`, aliases, model prefixes, and unknown-model passthrough.
- Native `reqwest` / `rustls` transport.
- Connection pooling, timeouts, response limits, request limits, and concurrency limits.
- Mimo streaming text de-duplication.
- `doctor`, `config validate`, provider matrix, benchmark, and `/metrics`.
- Docker Compose, systemd, scripts, and GitHub Actions CI.
...
Reload VS Code, ask a simple question, and check ModelPort logs for `routing message request`.
...
| Symptom | Meaning | Fix |
| --- | --- | --- |
| Missing token at startup | Local auth token is not set | Set `MODELPORT_AUTH_TOKEN` and matching `ANTHROPIC_AUTH_TOKEN`. |
| Placeholder warning | A key or token is still a placeholder | Replace it with a real value. |
| `/v1/models` returns 401 | Client token missing or mismatched | Check `x-api-key` and `ANTHROPIC_AUTH_TOKEN`. |
| `INVALID_API_KEY` | Upstream was reached, but upstream key is invalid | Replace `MIMO_OPENAI_API_KEY`. |
| VS Code does not use ModelPort | Extension did not load environment variables | Restart VS Code and verify `ANTHROPIC_BASE_URL`. |
| Request timeout | Upstream or network is slow | Check upstream first, then tune timeout variables. |
...
- [docs/GPT_IMAGE_2_GUIDE.md](docs/GPT_IMAGE_2_GUIDE.md)
