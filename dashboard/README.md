...
RoutePilot uses consistent naming across the application, configuration, and deployment resources.

The dashboard is RoutePilot's browser control plane. It is built with React,
...
- A running RoutePilot backend for real mode.
...
Set `ROUTEPILOT_VITE_PROXY_TARGET` to another backend origin when isolating E2E
runs or developing against multiple local instances; the default is
`http://127.0.0.1:38082`.

Login uses `ROUTEPILOT_ADMIN_USERNAME` and `ROUTEPILOT_ADMIN_PASSWORD`. The router
...
CORS/credential policy. `ROUTEPILOT_ALLOWED_ORIGINS` only affects dashboard write
