...
export interface RoutePilotE2EEnv {
  adminUsername: string
  adminPassword: string
  authToken: string
}

export function routepilotEnv(): RoutePilotE2EEnv {
  const fileEnv = readEnvFile(process.env.ROUTEPILOT_ENV_FILE || path.resolve(process.cwd(), '..', '.env'))
  const env = { ...fileEnv, ...process.env }
  return {
    adminUsername: env.ROUTEPILOT_ADMIN_USERNAME || 'admin',
    adminPassword: env.ROUTEPILOT_ADMIN_PASSWORD || '',
    authToken: env.ROUTEPILOT_AUTH_TOKEN || env.ANTHROPIC_AUTH_TOKEN || '',
  }
}

export function requireE2EEnv(): RoutePilotE2EEnv {
  const env = routepilotEnv()
  if (!env.adminPassword) {
    throw new Error('ROUTEPILOT_ADMIN_PASSWORD is required for dashboard E2E tests')
  }
  if (!env.authToken) {
    throw new Error('ROUTEPILOT_AUTH_TOKEN or ANTHROPIC_AUTH_TOKEN is required for dashboard E2E tests')
  }
  return env
}
...
export async function seedFailedGatewayRequest(page: Page, env = requireE2EEnv()) {
  ...
  const response = await page.request.post('/v1/chat/completions', {
    ...
    data: {
      ...
      messages: [{ role: 'user', content: 'RoutePilot isolated E2E ledger seed' }],
    },
  })
