...
test.describe('request logs', () => {
  ...
  test('keeps request queue and detail side by side on desktop', async ({ page }) => {
    await seedFailedGatewayRequest(page)
    await page.setViewportSize({ width: 1440, height: 1000 })
    await page.goto('/logs')
    await page.getByRole('button', { name: /查看 .* 请求详情/ }).first().click()
    const queue = await page.getByRole('region', { name: '请求列表' }).boundingBox()
    const detail = await page.getByRole('region', { name: '请求详情' }).boundingBox()
    expect(queue && detail && queue.x + queue.width <= detail.x).toBeTruthy()
  })
})
