...
test.describe('users and API keys', () => {
  ...
  test('admin can see, edit, and clean up a user and API key', async ({ page }) => {
    ...
    const email = `${username}@routepilot.local`
    const updatedEmail = `${username}+updated@routepilot.local`
    ...
    await page.getByText('密钥横向比较与完整列表', { exact: true }).click()
