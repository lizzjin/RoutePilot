    ...
    <TooltipProvider delayDuration={0}>
      ...
      >
        ...
        <div className="flex h-14 items-center gap-2.5 border-b border-sidebar-border px-3">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-border bg-primary text-primary-foreground shadow-none">
            <BrandMark className="h-7 w-7" />
          </div>
          {!isCollapsed && (
            <div className="min-w-0">
              <span className="block text-lg font-bold leading-5 tracking-tight">RoutePilot</span>
              <span
            ...
            {NAV_SECTIONS.map((section) => {
              ...
              return (
                <div key={section} className="space-y-1">
                  {!isCollapsed && (
                    <p className="px-2.5 pb-1 text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">
                      {section}
                    </p>
                  )}
              ...
              const link = (
                <NavLink
                  ...
                  className={cn(
                    ...
                    isActive
                      ? 'bg-sidebar-accent font-semibold text-sidebar-primary before:bg-sidebar-primary'
                      : 'text-body hover:bg-sidebar-accent/50 hover:text-sidebar-foreground',
                  )}
                >
                  {Icon && (
                    <Icon
                      className={cn(
                        ...
                        isActive ? 'text-sidebar-primary' : 'text-muted-foreground group-hover:text-sidebar-foreground/80',
                      )}
                    />
                  )}
        ...
        <div className="flex items-center justify-between p-2">
          {!isCollapsed && (
            <div className="flex min-w-0 items-center gap-2 px-1 text-xs text-muted-foreground" title={processStatus.title}>
              <span className={cn('h-2 w-2 shrink-0 rounded-full shadow-none', processStatus.kind === 'online' ? 'bg-success text-success-text' : processStatus.kind === 'error' ? 'bg-destructive text-destructive-text' : 'bg-warning text-warning-text')} />
