...
describe('api client', () => {
  ...
  it('adds JSON and CSRF headers to write requests', async () => {
    ...
    expect(headers.get('X-RoutePilot-CSRF')).toBe('1')
  })
  ...
  it('isolates cached data and remembers the protected route after a 401', async () => {
    ...
    expect(values.get('routepilot_return_to')).toBe('/logs?status=error#request')
    expect(values.get('routepilot_auth_notice')).toContain('会话已过期')
