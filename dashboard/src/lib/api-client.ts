...
const CHANGE_REQUEST_STORAGE_KEY = 'routepilot_change_request_id'
...
async function request<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!['GET', 'HEAD'].includes(method) && !headers.has('X-RoutePilot-CSRF')) {
    headers.set('X-RoutePilot-CSRF', '1')
  }
  if (!['GET', 'HEAD'].includes(method) && !headers.has('X-RoutePilot-Change-Request-Id')) {
    const approvedChange = typeof window === 'undefined'
      ? ''
      : window.sessionStorage.getItem(CHANGE_REQUEST_STORAGE_KEY)?.trim() || ''
    if (approvedChange) headers.set('X-RoutePilot-Change-Request-Id', approvedChange)
  }
  ...
  if (response.status === 401 && typeof window !== 'undefined') {
    if (window.location.pathname !== '/login') {
      clearSessionQueries()
      try {
        window.sessionStorage.setItem('routepilot_auth_notice', '会话已过期，请重新登录后继续。')
        window.sessionStorage.setItem(
          'routepilot_return_to',
