...
export const mockDashboardStats: DashboardStats = {
  ...
  modelUsage: [
    { model: 'deepseek-v4-flash', provider: 'deepseek', requests: 18500, tokens: 18_500_000, cost: 9.25 },
    { model: 'mimo-v2.5-pro', provider: 'mimo', requests: 12300, tokens: 12_300_000, cost: 6.15 },
    { model: 'claude-sonnet-4-20250514', provider: 'anthropic', requests: 8900, tokens: 8_900_000, cost: 44.5 },
  ],
  tokenTimeSeries: generateTimeSeries(24, 80_000, 30_000).map((point) => ({
    timestamp: point.timestamp,
    inputTokens: Math.round(point.value * 0.55),
    outputTokens: Math.round(point.value * 0.3),
    cacheWriteTokens: Math.round(point.value * 0.05),
    cacheReadTokens: Math.round(point.value * 0.1),
    cacheHitRate: 14.3,
  })),
  rangeSummary: {
    totalRequests: 56780,
    successRequests: 54963,
    totalInputTokens: 31_000_000,
    totalOutputTokens: 16_000_000,
    totalCacheWriteTokens: 2_000_000,
    totalCacheReadTokens: 5_000_000,
    totalTokens: 54_000_000,
    totalCostEstimate: 59.9,
    totalActualCost: 16.2,
    totalBillableCost: 16.2,
    billableRequests: 18_500,
    estimateOnlyRequests: 38_280,
    rpm: 39.4,
    tpm: 37_500,
  },
  ...
  recentActivity: [
    ...
    { id: 'act_002', timestamp: '2026-06-10T09:28:00Z', type: 'account_issue', message: '供应商 deepseek 账号 main 余额不足，已标记为等待充值', severity: 'warning' },
