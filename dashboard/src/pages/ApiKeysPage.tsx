import { ObjectBrowser } from '@/components/shared/ObjectBrowser'
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-md border bg-card p-4 shadow-none">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          {(expiringKeys > 0 || expiredKeys > 0) && (
            <div className="inline-flex h-8 items-center gap-1.5 rounded-md border border-warning-text/30 bg-warning-soft px-3 text-xs text-warning-text">
      ...
      <Card className="overflow-hidden">
        ...
        <CardContent className="p-0">{keyWindow.items.length > 0 && <ObjectBrowser items={keyWindow.items} label={key => key.name} summary={key => <>{key.keyPreview || key.keyPrefix} · {key.status}</>} onSelect={closeEditDialog} renderDetail={key => <><h2 className="text-xl font-semibold">{key.name}</h2><p className="mt-2 text-xs text-muted-foreground">权限、用量与生命周期操作。明文只在签发或轮换时独立交接。</p><ApiKeyMobileCard
                  ...
                  /></>} />}
<details className="workbench-metadata" open={keyWindow.items.length === 0}><summary>密钥横向比较与完整列表</summary><CardContent className="p-0">
                ...
                <TableRow key={key.id} className="h-[94px]">
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1.5">
                      <code className="rounded-md bg-info-soft px-2.5 py-1 text-xs font-semibold text-info-text">
                    ...
                    <div className="flex justify-center gap-1">
                      ...
                      {!key.supersededByKeyId && ((key.status === 'active' && access.canRevoke) || (key.status === 'revoked' && access.canRestore)) && (
                        <ActionButton
                          ...
                          className={key.status === 'active' ? undefined : 'text-success-text hover:text-success-text'}
                      ...
                      {access.canDelete && (
                        <Button
                          variant="ghost"
                          size="sm"
                          className="h-12 w-11 flex-col gap-1 px-1 text-xs text-destructive-text"
        ...
        >
          ...
          {newKey ? (
            <div className="min-w-0 space-y-4">
              <div role="status" className="rounded-md border border-warning-text/30 bg-warning-soft p-3 text-sm text-warning-text">
            ...
            <div className="space-y-4">
              ...
              {isAdmin ? <div className="space-y-2">
                <Label htmlFor="create-key-user">用户 <span className="text-destructive-text" aria-hidden="true">*</span></Label>
              ...
              <div className="space-y-2">
                <Label htmlFor="create-key-name">名称 <span className="text-destructive-text" aria-hidden="true">*</span></Label>
              ...
              {isAdmin && <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  {serviceAccountExpiryIssue && <p id="create-key-expires-error" role="alert" className="text-xs text-destructive-text">{serviceAccountExpiryIssue}</p>}
                </div>
              </div>}
              {isAdmin && form.principalType === 'service_account' && (
                <div className="space-y-2 rounded-lg border border-warning-text/30 bg-warning-soft p-3">
                  <Label htmlFor="create-key-purpose">自动化用途 <span className="text-destructive-text" aria-hidden="true">*</span></Label>
      ...
      {access.canEdit && (
        <Editor.Root open={!!editingKey} onOpenChange={(open) => { if (!open) closeEditDialog() }}>
          <Editor.Content className="max-h-[90vh] max-w-[640px] gap-0 overflow-hidden p-0 sm:rounded-lg">
          <Editor.Header className="border-b px-7 py-5">
            <Editor.Title className="text-xl">编辑密钥</Editor.Title>
            <Editor.Description className="sr-only">更新 API 密钥设置</Editor.Description>
          </Editor.Header>
            ...
            {isAdmin && (
              <>
                <div className="space-y-2">
                  ...
                  >
                    <SelectTrigger id="edit-key-team" className="h-12 rounded-md bg-background">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={editForm.status} onValueChange={(value) => updateEditForm({ status: value as ApiKey['status'] })}>
                    <SelectTrigger id="edit-key-status" className="h-12 rounded-md bg-background">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-2">
                  ...
                  />
                  {editExpiresInPast && (
                    <p role="alert" className="text-xs text-destructive-text">该时间已过去；请选择未来时间、清空有效期或将状态设为“已禁用”。</p>
                  )}
                </div>
              </>
            )}
          ...
          <Editor.Footer className="border-t bg-background px-7 py-5">
          ...
          {rotationResult && (
            <div className="space-y-4">
              ...
              {rotationTerminalMessage ? (
                <div role="alert" className="rounded-md border border-warning-text/30 bg-warning-soft p-3 text-sm text-warning-text">
...
function EndpointPill({
  ...
  return (
    <div className="inline-flex h-8 max-w-full items-center gap-2 rounded-md border bg-background px-3 text-xs shadow-none">
      <span className="font-medium text-foreground">{label}</span>
      {tag && (
        <span className="rounded bg-success-soft px-1.5 py-0.5 font-medium text-success-text">
          {tag}
        </span>
      )}
...
function CopySnippet({ title, value, onCopy }: { title: string; value: string; onCopy: (value: string) => void }) {
  return (
    <div className="min-w-0 overflow-hidden rounded-md border bg-background">
      ...
      <pre className="overflow-x-auto whitespace-pre p-3 text-xs leading-5 text-muted-foreground">{value}</pre>
    </div>
  )
}
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      {teamName && (
        <Badge
          variant="outline"
          className="border-info-text/30 bg-info-soft font-medium text-info-text shadow-none"
        >
      ...
      {group && (
        <Badge
          variant="outline"
          className="border-success-text/30 bg-success-soft font-medium text-success-text shadow-none"
        >
    ...
    <article className="space-y-4 p-4">
      ...
      {(canEdit || canRotate || canToggle || canDelete) && (
        <div className="flex flex-wrap gap-2 border-t pt-3">
          ...
          {canToggle && (
            <Button variant="outline" size="sm" disabled={pending} onClick={onToggle} className={apiKey.status === 'active' ? 'text-destructive' : 'text-success-text'}>
              {apiKey.status === 'active' ? <ShieldOff className="mr-1.5 h-4 w-4" /> : <ShieldCheck className="mr-1.5 h-4 w-4" />}
              {apiKey.status === 'active' ? '禁用' : '恢复'}
            </Button>
          )}
          {canDelete && <Button variant="ghost" size="sm" disabled={pending} onClick={onDelete} className="ml-auto text-destructive-text"><Trash2 className="mr-1.5 h-4 w-4" />删除</Button>}
        </div>
      )}
    </article>
  )
...
function ExpiresCell({ value, now }: { value: string | null; now: number }) {
  ...
  return (
    <div className={cn('flex items-center gap-2 text-sm', expiry === 'expired' ? 'text-destructive-text' : expiry === 'expiring' ? 'text-warning-text' : undefined)}>
...
function EffectiveStatusBadge({ apiKey, now }: { apiKey: ApiKey; now: number }) {
  ...
  if (apiKey.status === 'active' && expiry === 'expiring') {
    return <Badge variant="outline" className="border-warning-text/30 bg-warning-soft text-warning-text">即将过期</Badge>
  }
  if (apiKey.status === 'active') {
    return (
      <Badge
        variant="outline"
        className="border-success-text/30 bg-success-soft text-success-text shadow-none"
      >
        活跃
      </Badge>
    )
...
function ErrorNotice({ message, actionLabel, onAction }: { message: string; actionLabel?: string; onAction?: () => void }) {
  ...
  return (
    <div role="alert" className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-destructive/25 bg-destructive/10 px-3 py-2 text-sm text-destructive-text">
      <span>{message}</span>
