...
export function EnterprisePage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <details className="workbench-metadata"><summary>账本全局统计</summary><OverviewRail overview={overviewQuery.data} loading={overviewQuery.isLoading} /></details>

      {overviewQuery.data && <RuntimeStrip overview={overviewQuery.data} />}

      <details className="workbench-metadata"><summary>事务预算与证据操作</summary><BudgetRail
      ...
      {(overviewQuery.isError || requestsQuery.isError) && (
        <div className="flex items-center gap-2 border-y border-warning-text/30 bg-warning-soft px-3 py-2 text-sm text-warning-text">
...
<div className={`workbench-split ledger-workbench ${selectedLedgerId ? 'has-selection' : ''}`}><div className="ledger-list-pane">      <section aria-label="受治理请求记录" className="overflow-hidden border-y">
        <div className="flex items-center justify-between border-b bg-muted/20 px-3 py-2.5">
          ...
          {overviewQuery.data?.activeLeases ? (
            <Badge variant="outline" className="gap-1.5 border-info-text/30 bg-info-soft text-info-text">
        ...
        {requestsQuery.isLoading ? (
          ...
          <><div className="workbench-directory">{requests.map(request => <button key={request.ledgerId} aria-pressed={selectedLedgerId === request.ledgerId} onClick={() => setSelectedLedgerId(request.ledgerId)}><span><strong>{request.requestedModel}</strong><small>{request.state} · {request.statusCode ?? '未结束'} · {durationLabel(request)}</small><small>{request.ledgerId}</small></span></button>)}</div><details className="workbench-metadata"><summary>横向比较请求事实</summary><Table>
            <TableHeader>
      ...
      </div><div className="ledger-detail-pane">{!selectedLedgerId && <div className="workbench-empty">选择账本请求，查看每次 Provider 尝试与计费证据。</div>}
      <RequestDetailDrawer
...
function RuntimeStrip({ overview }: { overview: EnterpriseLedgerOverview }) {
  ...
  return (
    <section className="flex flex-col gap-3 border-l-2 border-border bg-muted/20 px-4 py-3 text-xs lg:flex-row lg:items-center lg:justify-between">
      <div className="flex min-w-0 items-center gap-3">
        <Database className={cn('h-4 w-4 shrink-0', relational ? 'text-success-text' : 'text-warning-text')} />
    ...
    <section aria-label="预算控制" className="border-y">
      <div className="grid lg:grid-cols-[minmax(230px,1.3fr)_repeat(4,minmax(120px,0.7fr))_auto] lg:divide-x">
        <div className="flex min-w-0 items-center gap-3 px-4 py-4">
          <CircleDollarSign className="h-5 w-5 shrink-0 text-foreground" />
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <h2 className="text-sm font-semibold">事务预算控制</h2>
              <Badge variant="outline" className="h-5 border-success-text/30 bg-success-soft px-1.5 text-xs text-success-text">硬门禁</Badge>
            </div>
        ...
        <BudgetMetric label="已预留" value={loading ? undefined : formatMoney(account?.reservedMicrounits ?? 0)} tone={(account?.reservedMicrounits ?? 0) > 0 ? 'text-info-text' : undefined} />
        <BudgetMetric label="已结算" value={loading ? undefined : formatMoney(account?.settledMicrounits ?? 0)} />
        <BudgetMetric label="可用" value={loading ? undefined : finite ? formatMoney(account.availableMicrounits ?? 0) : '不限额'} tone={(account?.availableMicrounits ?? 0) < 0 ? 'text-destructive-text' : undefined} />
        <div className="flex items-center justify-between gap-3 border-t px-4 py-3 lg:border-t-0">
          <div className="w-24">
            <div className="mb-1.5 flex justify-between text-xs text-muted-foreground"><span>占用</span><span>{finite ? `${percent.toFixed(1)}%` : '∞'}</span></div>
            <div className="h-1.5 overflow-hidden rounded-full bg-muted"><div className={cn('h-full rounded-full', percent >= 90 ? 'bg-destructive' : percent >= 70 ? 'bg-warning' : 'bg-foreground')} style={{ width: finite ? `${percent}%` : '0%' }} /></div>
          </div>
      ...
      {error && <p className="border-t px-4 py-2 text-xs text-warning-text">预算状态暂时无法刷新，推理门禁仍由服务端独立执行。</p>}
    </section>
  )
}
...
function BudgetEventRow({ event }: { event: EnterpriseBudgetEvent }) {
  ...
  return (
    <div className="grid gap-2 border-b px-1 py-3 text-xs last:border-b-0 sm:grid-cols-[120px_minmax(0,1fr)_150px] sm:items-center">
      <div><p className="font-medium">{budgetEventLabel(event.eventType)}</p><p className="mt-1 text-xs text-muted-foreground">{formatDateTime(event.createdAtMs)}</p></div>
      <div className="min-w-0"><p className="truncate text-muted-foreground">{event.reason || event.evidenceSource}</p><p className="mt-1 truncate font-mono text-xs text-muted-foreground">{event.attemptId || event.actorId || event.eventId}</p></div>
        ...
        {amounts.map((amount) => (
          <div key={amount.dimension} className="flex items-baseline justify-end gap-2">
            ...
            <span className={cn(
              ...
              amount.dimension === 'reserved' && amount.microunits > 0 && 'text-info-text',
              amount.microunits < 0 && 'text-success-text',
              amount.dimension === 'settled' && amount.microunits > 0 && 'text-warning-text',
            )}>
              {formatSignedMoney(amount.microunits)}
            </span>
          </div>
        ))}
        <p className="text-xs text-muted-foreground">{event.evidenceSource}</p>
      </div>
    </div>
  )
}
    ...
    <TableRow className="group cursor-pointer" onClick={onSelect}>
      <TableCell className="pl-3">
        <button type="button" className="block max-w-[180px] text-left" onClick={onSelect}>
          <span className="block truncate font-mono text-xs font-medium text-foreground">{shortId(request.ledgerId)}</span>
          <span className="mt-1 block truncate font-mono text-xs text-muted-foreground">{shortId(request.requestId)}</span>
        </button>
      </TableCell>
      <TableCell>
        <p className="max-w-52 truncate text-xs font-medium">{request.organizationId} / {request.projectId}</p>
        <p className="mt-1 max-w-52 truncate text-xs text-muted-foreground">{request.environmentId} · {request.principalId}</p>
      </TableCell>
      <TableCell>
        <p className="max-w-48 truncate text-xs font-medium">{request.requestedModel}</p>
        <p className="mt-1 text-xs text-muted-foreground">{protocolLabel(request.clientProtocol)} · {request.stream ? 'SSE' : 'JSON'} · {trafficClassLabel(request.trafficClass)}</p>
      </TableCell>
      <TableCell>
        <div className="flex flex-wrap items-center gap-1.5">
          <StateBadge state={request.state} />
          {request.hasIdempotencyKey && <Badge variant="outline" className="h-5 px-1.5 text-xs">幂等</Badge>}
        </div>
        <p className="mt-1 text-xs text-muted-foreground">{leaseLabel(request)} · {request.attemptCount} attempts</p>
      </TableCell>
      ...
      <TableCell className="pr-3 text-right">
        ...
        <p className="mt-1 text-xs text-muted-foreground">{durationLabel(request)}</p>
      </TableCell>
    </TableRow>
  )
}
...
function RequestDetailDrawer({
  ...
  if (!open) return null
  return (
    <section className="border bg-card" aria-label="请求事实">
      <div className="p-5"><Button variant="outline" size="sm" className="mb-4" onClick={() => onOpenChange(false)}>返回账本列表</Button>
        <div className="mb-5">
          <h2 className="text-xl font-semibold">请求事实</h2>
          <p className="text-sm text-muted-foreground">Gateway Request 与每次 Provider Attempt 的持久化终态。</p>
        </div>
        {loading ? (
  ...
  return (
    <div>
          ...
          <div className="relative space-y-0 before:absolute before:bottom-5 before:left-[7px] before:top-5 before:w-px before:bg-border">
            {attempts.map((attempt, index) => (
              <div key={attempt.attemptId} className="relative flex gap-4 py-4 first:pt-0">
                <span className={cn('relative z-[1] mt-1 h-[15px] w-[15px] shrink-0 rounded-full border-4 border-background', attempt.state === 'completed' ? 'bg-success' : attempt.state === 'started' ? 'bg-info' : 'bg-destructive')} />
                <div className="min-w-0 flex-1 border-b pb-4">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="mt-1 font-mono text-xs text-muted-foreground">#{index + 1} · {attempt.attemptId}</p>
                    </div>
      ...
      {request.errorMessage && (
        <div className="mx-6 mb-8 border-l-2 border-destructive bg-destructive/5 px-4 py-3 text-sm text-destructive-text">
...
function UsageCell({ label, value }: { label: string; value: number }) {
  return <div className="bg-background px-3 py-3"><p className="text-xs text-muted-foreground">{label}</p><p className="mt-1 font-mono text-sm font-semibold">{value.toLocaleString('zh-CN')}</p></div>
}

function StateBadge({ state }: { state: EnterpriseRequestState }) {
  return <Badge variant="outline" className={cn('h-5 px-1.5 text-xs', stateTone(state))}>{stateLabel(state)}</Badge>
}
...
function stateTone(state: EnterpriseRequestState) {
  if (state === 'completed') return 'border-success-text/30 bg-success-soft text-success-text'
  if (state === 'started') return 'border-info-text/30 bg-info-soft text-info-text'
  if (state === 'cancelled') return 'border-warning-text/30 bg-warning-soft text-warning-text'
  return 'border-destructive-text/30 bg-destructive-soft text-destructive-text'
}
