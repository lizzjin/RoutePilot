...
export function GovernancePage() {
  ...
  useNavigationProtection(formDirty, { title: '变更意图尚未提交', description: '离开将丢弃当前未提交的变更载荷和原因。' })
  const [payloadText, setPayloadText] = useState(() => formatTemplate(ACTIONS[0][0]))
  const [notice, setNotice] = useState<{ kind: 'success' | 'error'; text: string } | null>(null)
  const [selectedChangeId, setSelectedChangeId] = useState(() => window.sessionStorage.getItem('routepilot_change_request_id') || '')
  ...
  return (
    <div className="space-y-6">
      ...
      <details className="workbench-metadata"><summary>治理状态与交互队列</summary><div className="grid gap-4 md:grid-cols-4">
      ...
      {notice && (
        <div className={`rounded-lg border p-3 text-sm ${notice.kind === 'success' ? 'border-success-text/30 bg-success-soft text-success-text' : 'border-destructive-text/30 bg-destructive-soft text-destructive-text'}`} role="status">
          {notice.text}
        </div>
      )}

      {selectedChangeId && (
        <div className="flex flex-col gap-2 rounded-lg border border-info-text/30 bg-info-soft p-3 text-sm text-info-text sm:flex-row sm:items-center sm:justify-between" role="status">
          <span>专用操作已选择审批单 <code>{selectedChangeId}</code>；后续 Dashboard 写请求会自动携带该 ID，服务端仍会校验动作、目标与载荷摘要。</span>
          <Button size="sm" variant="outline" onClick={() => {
            window.sessionStorage.removeItem('routepilot_change_request_id')
            setSelectedChangeId('')
          }}>清除选择</Button>
        </div>
      )}
      ...
      <><Card>
        ...
        <CardContent className="space-y-3">
          ...
          <ObjectBrowser items={data.changeRequests} label={change => actionLabels.get(change.action as typeof ACTIONS[number][0]) || change.action} summary={change => <>{change.target} · {change.status}</>} renderDetail={change => (<div key={change.id} data-testid={`governance-change-${change.id}`} className="rounded-md border border-border/80 p-4">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div className="min-w-0 space-y-1">
                  ...
                  <p className="break-all font-mono text-xs text-muted-foreground">SHA-256 {change.payloadSha256}</p>
                ...
                <div className="flex shrink-0 gap-2">
                  ...
                  {change.status === 'approved' && !DIRECT_APPLY_ACTIONS.has(change.action) && (
                    <Button size="sm" variant="outline" onClick={() => {
                      window.sessionStorage.setItem('routepilot_change_request_id', change.id)
      ...
      </Card><details className="workbench-metadata"><summary>提交高风险变更</summary><Card>
        <CardHeader>
        ...
        <CardContent className="grid gap-4" onChangeCapture={() => setFormDirty(true)}>
