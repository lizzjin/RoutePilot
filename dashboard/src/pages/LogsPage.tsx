...
export function LogsPage() {
  ...
  const [selectionSnapshot, setSelectionSnapshot] = useState<RequestLog | null>(null)
  const selectedRow = useRef<string | null>(null)
  const selectLog = (log: RequestLog) => {
    selectedRow.current = log.id
    setSelectionSnapshot(log)
    const next = new URLSearchParams(searchParams)
    next.set('request', log.id)
  ...
  const selectedLog = requestId
    ? selectedLogDetail ?? logs.find(log => log.id === requestId) ?? (selectionSnapshot?.id === requestId ? selectionSnapshot : null)
    : null
  ...
  const closeDrawer = useCallback(() => {
    setSelectionSnapshot(null)
    setSearchParams(current => { const next = new URLSearchParams(current); next.delete('request'); return next }, { replace: true })
  }, [setSearchParams])
  useEffect(() => {
    if (requestId || !selectedRow.current) return
    let frame = requestAnimationFrame(() => {
      frame = requestAnimationFrame(() => {
        if (selectedRow.current) document.querySelector<HTMLElement>(`[data-log-id="${CSS.escape(selectedRow.current)}"]`)?.focus()
      })
    })
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="min-w-0 space-y-1">
            <div className="flex flex-wrap items-center gap-2">
              <Badge variant="outline" className="gap-1.5 border-success-text/30 bg-success-soft text-success-text">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              ...
              className={cn(
                ...
                liveMode && 'bg-success-soft hover:bg-success-soft text-success-text',
              )}
      ...
      {liveMode && (
        <div className="rounded-lg border border-success-text/30 bg-success-soft px-4 py-2 text-xs text-success-text" role="status">
          每 3 秒刷新当前结果。为接收新请求，已移除固定结束时间；切换到后台标签页时会暂停请求。
        </div>
      )}

      {isError && data && (
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-warning-text/30 bg-warning-soft px-4 py-3 text-sm text-warning-text" role="alert">
      ...
      {/* Summary cards */}
      <details className="workbench-metadata"><summary>完整筛选结果的用量与成本汇总（估算）</summary><LogsSummaryGrid summary={summary} /></details>
      ...
      <div className={`workbench-split logs-workbench ${requestId ? 'has-selection' : ''}`}><div className="logs-list-pane">
      {/* Table */}
      <LogsTable
        logs={logs}
        total={total}
      ...
      </div><div className="logs-detail-pane">
      {!requestId && <div className="workbench-empty">选择请求，查看路由、结果及费用。连续选择列表项即可切换详情。</div>}
      {requestId && !selectedLog && <section aria-label="请求详情" className="border bg-card p-5"><Button variant="outline" onClick={closeDrawer}>返回请求列表</Button>{selectedLogError ? <ErrorState title="请求详情加载失败" message="无法读取该请求；它可能已被清理或当前账号没有访问权限。" onRetry={() => void refetchSelectedLog()} /> : <LoadingPage />}</section>}
      {/* Detail panel */}
      <LogsDrawer
