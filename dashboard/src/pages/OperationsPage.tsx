import { useNavigationProtection } from '@/components/shared/navigation-protection'
...
const severityClass: Record<OpsSeverity, string> = {
  'SEV-1': 'border-destructive-text/30 bg-destructive-soft text-destructive-text',
  'SEV-2': 'border-warning-text/30 bg-warning-soft text-warning-text',
  'SEV-3': 'border-warning-text/30 bg-warning-soft text-warning-text',
  'SEV-4': 'border-border bg-muted text-muted-foreground',
}
...
export function OperationsPage() {
  ...
  useNavigationProtection(configurationDraft !== null, { title: 'Agent 配置尚未保存', description: '离开将丢弃当前 Agent 配置修改。' })
  ...
  return (
    <div className="space-y-6">
      ...
      <details className="workbench-metadata"><summary>事件与 Agent 状态摘要</summary><div className="metric-grid grid gap-px overflow-hidden rounded-md border bg-border-soft md:grid-cols-2 xl:grid-cols-4">
      ...
      <div className={`workbench-split operations-workbench ${selectedId ? 'has-selection' : ''}`}><div className="operations-list-pane">      <Card>
        <CardHeader>
        ...
        <CardContent>
          {incidents.data.items.length === 0 ? (
            ...
            <><div className="workbench-directory">{incidents.data.items.map(incident => <button key={incident.id} aria-pressed={selectedId === incident.id} onClick={() => setSelectedId(incident.id)}><span><strong>{incident.title}</strong><small>{incident.severity} · {statusLabel[incident.status]} · {incident.occurrenceCount} 次</small><small>{formatTime(incident.lastSeenAtMs)}</small></span></button>)}</div><details className="workbench-metadata"><summary>横向比较事件</summary><div className="overflow-x-auto rounded-lg border">
              <Table>
                <TableHeader>
                  <TableRow>
      ...
      </div><div className="operations-detail-pane">{!selectedId && <div className="workbench-empty">选择事件，核对证据、处理操作与时间线。</div>}
      {selectedId && (
        <Card>
          <CardHeader>
      ...
      )}</div></div><details className="workbench-metadata"><summary>Agent 配置与模型诊断</summary><Card>
        <CardHeader>
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
            ...
            <Badge variant="outline" className={agentConfiguration.enabled ? 'border-success-text/30 text-success-text' : ''}>
            ...
            <div className="space-y-4 rounded-lg border p-4">
              ...
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">需要容器中的独立 ROUTEPILOT_OPS_MODEL_API_KEY；失败不会阻断规则告警。</p>
                </div>
                <Switch
...
function ModelAnalysis({ evidence }: { evidence: OpsIncidentEvidence[] }) {
  ...
  return (
    <div className="mt-4 rounded-md border border-info-text/30 bg-info-soft p-3 text-info-text">
      ...
      <p className="mt-2 text-xs text-info-text">该内容由模型根据脱敏事实生成，必须由人工验证；它不会触发任何操作。</p>
    </div>
  )
}
