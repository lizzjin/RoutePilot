...
export function QuotasPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <details className="workbench-metadata"><summary>配额状态摘要（不同单位分别统计）</summary><div className="metric-grid grid grid-cols-1 gap-px overflow-hidden rounded-md border bg-border-soft sm:grid-cols-2 xl:grid-cols-4" aria-label="配额概览">
        ...
        <MetricCard title="需要关注" value={warningQuotas} icon={AlertTriangle} description="使用率已达 80%" className={warningQuotas > 0 ? 'border-warning-text/30' : undefined} />
        <MetricCard title="已阻止调用" value={exhaustedQuotas} icon={Ban} description="额度为 0 或使用率已达 100%" className={exhaustedQuotas > 0 ? 'border-destructive-text/30' : undefined} />
      </div></details>
      ...
      <Card className="overflow-hidden">
        ...
        <CardContent className="p-0">{quotaWindow.items.length > 0 && <ObjectBrowser items={quotaWindow.items} label={quota => quota.username} summary={quota => <>{quota.quotaType} · {quota.period} · {quota.used} / {quota.limit}</>} onSelect={() => { setEditingQuota(null); setShowCreateDialog(false) }} renderDetail={quota => <><h2 className="text-xl font-semibold">{quota.username} 的配额规则</h2><p className="mt-2 text-sm text-muted-foreground">一条规则只限制一个用量类型与周期；0 表示阻止调用，删除表示解除单独限制。</p><QuotaMobileCard quota={quota} onEdit={openEditQuotaDialog} onDelete={setQuotaToDelete} /></>} />}
<details className="workbench-metadata" open={quotaWindow.items.length === 0}><summary>规则横向比较与完整列表</summary><CardContent className="p-0">
      ...
      >
        <Editor.Content>
          <Editor.Header>
            <Editor.Title>{editingQuota ? '调整配额限额' : '新建配额'}</Editor.Title>
            <Editor.Description>{editingQuota ? '为保证用量统计口径一致，编辑时只调整限额。' : '一条规则只限制一个用户、一个用量类型和一个 UTC 周期。'}</Editor.Description>
          </Editor.Header>
          <div className="space-y-4">
            {(createQuota.error || updateQuota.error) && <ErrorNotice message={errorMessage(createQuota.error || updateQuota.error)} />}
            <div className="space-y-2">
              <Label htmlFor="quota-user">用户 <span className="text-destructive-text" aria-hidden="true">*</span></Label>
              <Select
            ...
            <div className="space-y-2">
              <Label htmlFor="quota-limit">限额 <span className="text-destructive-text" aria-hidden="true">*</span></Label>
              <Input
                id="quota-limit"
                required
                type="number"
              ...
              {form.limit.trim() && !isQuotaLimitValid(parsedLimit, form.quotaType) && (
                <p role="alert" className="text-xs text-destructive-text">请输入有效的{form.quotaType === 'cost' ? '非负金额' : '非负整数'}。</p>
              )}
              {form.limit.trim() && parsedLimit === 0 && (
                <div role="alert" className="rounded-md border border-destructive-text/30 bg-destructive-soft px-3 py-2 text-xs leading-relaxed text-destructive-text">
    ...
    <TableRow>
      ...
      <TableCell>
        <div className="flex items-center">
          ...
          <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive-text" onClick={() => onDelete(quota)} aria-label={`删除 ${quota.username} 的${quotaTypeLabels[quota.quotaType]}配额`}>
    ...
    <article className="space-y-4 p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center">
          ...
          <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive-text" onClick={() => onDelete(quota)} aria-label={`删除 ${quota.username} 的${quotaTypeLabels[quota.quotaType]}配额`}><Trash2 className="h-4 w-4" /></Button>
        </div>
      </div>
      <div>
  ...
  return (
    <div className="flex items-center gap-2">
      <Progress
        ...
        className={cn('flex-1', risk === 'exhausted' ? '[&>div]:bg-destructive' : risk === 'warning' ? '[&>div]:bg-warning' : '[&>div]:bg-success')}
      />
      <span className={cn('w-12 text-right text-sm font-semibold tabular-nums', risk === 'exhausted' ? 'text-destructive-text' : risk === 'warning' ? 'text-warning-text' : 'text-success-text')}>
        {blocked ? '阻止' : `${percent}%`}
      </span>
    </div>
  )
