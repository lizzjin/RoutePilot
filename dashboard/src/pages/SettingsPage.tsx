...
function SettingsForm({ initialSettings }: { initialSettings: SystemSettings }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {!canOperate && (
        <div className="flex items-start gap-3 rounded-lg border border-info-text/30 bg-info-soft p-4 text-sm text-info-text" role="status">
      ...
      <details className="workbench-metadata"><summary>运行状态摘要</summary><SettingsOverview
        settings={form}
        providers={providers}
      ...
      <details className="workbench-metadata"><summary>首次接入检查</summary><SetupChecklist
        setup={form.setup}
      ...
      <Tabs className="workbench-section-nav" value={activeTab} onValueChange={(value) => setActiveTab(value as SettingsOperatorTab)}>
          ...
          <div className="space-y-4">
            <Card>
              ...
              <CardContent className="grid gap-3 md:grid-cols-3">
                <RuntimeFact label="绑定地址" value={form.server.bindAddress} hint="ROUTEPILOT_BIND" mono />
                <RuntimeFact label="最大请求体" value={formatBytes(form.server.maxRequestBodyBytes)} hint="ROUTEPILOT_MAX_REQUEST_BODY_BYTES" />
                <RuntimeFact label="并发请求" value={String(form.server.maxConcurrentRequests)} hint="ROUTEPILOT_MAX_CONCURRENT_REQUESTS" />
              </CardContent>
            </Card>
...
function SmartRoutingStatusCard({
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        {error && (
          <div className="flex items-center gap-2 rounded-lg border border-destructive-text/30 bg-destructive-soft p-3 text-sm text-destructive-text">
  ...
  return (
    <Card>
      ...
      <CardContent>
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {checks.map((check) => (
            ...
            >
              ...
              <span className="text-xs text-foreground">查看</span>
            </button>
          ))}
        </div>
        {issues.length > 0 && (
            ...
            {issues.map((issue, index) => (
              <div
                ...
                className={issue.severity === 'error'
                  ? 'flex items-start gap-2 rounded-md border border-destructive-text/30 bg-destructive-soft p-3 text-xs text-destructive-text'
                  : 'flex items-start gap-2 rounded-md border border-warning-text/30 bg-warning-soft p-3 text-xs text-warning-text'}
              >
        ...
        {warningCount > 0 && <p className="mt-3 text-xs text-warning-text">共 {warningCount} 条警告；运行可用不代表每个 Provider 都已验收。</p>}
      </CardContent>
    </Card>
  )
}
    ...
    <Card className={className}>
      <CardHeader className="flex-col items-start justify-between gap-3 space-y-0 sm:flex-row">
        <div>
          ...
          <CardDescription className="mt-2">先预览，再按 30 / 90 / 395 天策略处理运维元数据；RoutePilot 不持久化提示词和响应正文。</CardDescription>
        </div>
        {result && (
  ...
  return (
    <div
      ...
      className={type === 'success'
        ? 'flex items-start gap-2 rounded-lg border border-success-text/30 bg-success-soft p-3 text-sm text-success-text'
        : 'flex items-start gap-2 rounded-lg border border-destructive-text/30 bg-destructive-soft p-3 text-sm text-destructive-text'}
    >
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border p-4">
      <div className="min-w-0 space-y-1">
        ...
        {lastTest && (
          <div className={lastTest.success ? 'text-xs text-success-text' : 'text-xs text-destructive-text'}>
        ...
        {!lastTest && readiness && (
          <p className={readiness.level === 'ready' ? 'text-xs text-success-text' : 'text-xs text-warning-text'}>
...
function statusIcon(status: SetupCheck['status'] | AuditEvent['severity']) {
  if (status === 'ok' || status === 'info') return <CheckCircle2 className="h-4 w-4 text-success-text" />
  if (status === 'warning') return <AlertTriangle className="h-4 w-4 text-warning-text" />
  return <CircleAlert className="h-4 w-4 text-destructive-text" />
}
...
function downloadBackup(backup: BackupExport) {
  ...
  anchor.href = url
  anchor.download = `routepilot-diagnostic-snapshot-${stamp}.json`
  anchor.click()
