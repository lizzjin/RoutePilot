  ...
  return (
    <div className="relative">
      <Button
        ...
        variant="ghost"
        size="sm"
        className="absolute right-2 top-2 z-10 h-8 border border-code-foreground/50 bg-code-background text-code-foreground hover:bg-code-foreground/10 hover:text-code-foreground"
        onClick={() => void copy()}
      ...
      <pre tabIndex={0} aria-label={copyLabel} className="overflow-x-auto rounded-md bg-code-background p-4 pr-24 font-mono text-xs leading-6 text-code-foreground">
        <code>{children}</code>
      </pre>
    </div>
  )
...
function GuideStep({
  ...
  const content = (
    <>
      <div className="flex items-center justify-between gap-3">
        <span className="font-mono text-xs font-semibold text-foreground">0{index}</span>
  ...
  const className = 'group border-t pt-4 transition-colors hover:border-border'
...
function SectionHeading({
  ...
  return (
    <div className="mb-5">
      <p className="text-xs font-semibold uppercase tracking-[0.16em] text-foreground">{eyebrow}</p>
...
export function UsageGuidePage() {
  ...
  return (
    <div className="guide-workbench">
      ...
      <div className="workbench-queue-layout"><nav className="workbench-directory" aria-label="接入步骤">{['01 · 获取密钥与模型', '02 · 配置客户端', '03 · 验证请求与安全', ...(isAdmin ? ['04 · 管理员接入'] : [])].map((name,index) => <button key={name} aria-pressed={step === index} onClick={() => setStep(index)}>{name}</button>)}</nav><div className="min-w-0 border bg-card px-5">
      <section hidden={step !== 0} className="border-b py-8">
      ...
      <section hidden={step !== 1} className="border-b py-8">
        <SectionHeading
          eyebrow="Client setup"
          title="配置客户端"
          description="两种客户端协议使用同一套 RoutePilot API Key，但 Base URL 规则不同。"
        />
        <div className="mb-6 rounded-lg border bg-muted/20 p-4">
          ...
          {catalogError ? (
            <p className="mt-3 rounded-md border border-destructive-text/30 bg-destructive-soft px-3 py-2 text-xs text-destructive-text" role="alert">
            ...
            <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-md border border-warning-text/30 bg-warning-soft px-3 py-2 text-xs text-warning-text">
        ...
        <div className="grid grid-cols-1 gap-8 xl:grid-cols-2 xl:gap-0 xl:divide-x">
          <article className="min-w-0 xl:pr-8">
            <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
              <h3 className="flex items-center gap-2 font-semibold"><Terminal className="h-4 w-4 text-foreground" />Anthropic-compatible</h3>
              <span className="text-xs text-muted-foreground">Claude Code / Anthropic SDK</span>
            </div>
            <p className="mb-3 text-sm leading-6 text-muted-foreground">客户端连接 RoutePilot，而不是直接连接实际模型上游。</p>
          ...
          <article className="min-w-0 xl:pl-8">
            <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
              <h3 className="flex items-center gap-2 font-semibold"><Terminal className="h-4 w-4 text-foreground" />OpenAI-compatible</h3>
      ...
      <section hidden={step !== 2} className="grid gap-8 border-b py-8 lg:grid-cols-[1.35fr_0.65fr] lg:divide-x">
      ...
      {isAdmin && (
        <section hidden={step !== 3} className="border-b py-8">
          <SectionHeading
          ...
          <div className="divide-y border-y">
            <Link to="/models" className="group flex items-center gap-4 py-4">
              <Boxes className="h-5 w-5 shrink-0 text-foreground" />
            ...
            <Link to="/api-keys" className="group flex items-center gap-4 py-4">
              <KeyRound className="h-5 w-5 shrink-0 text-foreground" />
      ...
      <div className="flex justify-between gap-4 py-5"><Button variant="outline" disabled={step === 0} onClick={() => setStep(step-1)}>上一步</Button><Button disabled={step === (isAdmin ? 3 : 2)} onClick={() => setStep(step+1)}>下一步</Button></div></div></div>
