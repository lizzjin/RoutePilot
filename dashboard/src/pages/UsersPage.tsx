...
export function UsersPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <details className="workbench-metadata"><summary>用户状态摘要</summary><div className="metric-grid grid grid-cols-2 gap-px overflow-hidden rounded-md border bg-border-soft sm:grid-cols-3" aria-label="用户概览">
      ...
      <Card className="overflow-hidden">
        ...
        <CardContent className="p-0">{usersWindow.items.length > 0 && <ObjectBrowser items={usersWindow.items} label={user => user.username} summary={user => <>{user.email} · {ROLE_LABELS[user.role]} · {user.status}</>} onSelect={closeEditUser} renderDetail={user => <><div className="flex items-start justify-between"><div><h2 className="text-xl font-semibold">{user.username}</h2><p className="mt-1 text-sm text-muted-foreground">{user.email}</p></div><UserActionMenu user={user} canDelete={canDeleteUser(user)} onEdit={openEditUser} onKeys={openUserKeys} onDelete={requestDeleteUser} /></div><dl className="workbench-context"><div><dt>角色</dt><dd>{ROLE_LABELS[user.role]}</dd></div><div><dt>状态</dt><dd>{user.status}</dd></div><div><dt>关联密钥</dt><dd>{user.apiKeyCount}</dd></div><div><dt>24 小时请求</dt><dd>{formatNumber(user.requestCount24h)}</dd></div><div><dt>创建时间</dt><dd>{formatDate(user.createdAt)}</dd></div><div><dt>最后登录</dt><dd>{user.lastLoginAt ? formatDate(user.lastLoginAt) : '从未登录'}</dd></div></dl><div className="flex flex-wrap gap-3"><Button onClick={() => openEditUser(user)}>编辑用户</Button><Button variant="outline" onClick={() => openUserKeys(user.id)}>查看关联密钥</Button></div></>} />}
<details className="workbench-metadata" open={usersWindow.items.length === 0}><summary>用户横向比较与完整列表</summary><CardContent className="p-0">
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <FormField id="create-email" label="邮箱" required>
              ...
              {form.email && !isUserEmailValid(form.email) && <p role="alert" className="text-xs text-destructive-text">请输入有效的邮箱地址。</p>}
            </FormField>
      ...
      <Editor.Root open={!!editingUser} onOpenChange={(open) => { if (!open) closeEditUser() }}>
        <Editor.Content>
          <Editor.Header>
            <Editor.Title>编辑用户</Editor.Title>
            <Editor.Description>角色和状态变更会立即影响用户权限。</Editor.Description>
          </Editor.Header>
          <div className="space-y-4">
            ...
            <FormField id="edit-email" label="邮箱" required>
              ...
              {editForm.email && !isUserEmailValid(editForm.email) && <p role="alert" className="text-xs text-destructive-text">请输入有效的邮箱地址。</p>}
            </FormField>
            ...
            {editForm.password && (
              <div role="status" className="rounded-md border border-warning-text/30 bg-warning-soft px-3 py-2 text-xs leading-relaxed text-warning-text">
            ...
            {editingUser && editingUser.status === 'active' && editForm.status !== 'active' && (
              <div role="alert" className="rounded-md border border-warning-text/30 bg-warning-soft px-3 py-2 text-sm text-warning-text">
                    ...
                    <TableBody>
                      {userKeysWindow.items.map((key) => (
                        <TableRow key={key.id}>
                          ...
                          <TableCell>
                            {key.status === 'active' && (
                              <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive-text" onClick={() => requestRevokeKey(key.id, key.name)} aria-label={`禁用密钥 ${key.name}`}>
...
function SummaryCard({ label, value, hint, tone = 'default', className }: { label: string; value: number; hint: string; tone?: 'default' | 'warning'; className?: string }) {
  return (
    <Card className={cn(tone === 'warning' && 'border-warning-text/30', className)}>
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        ...
        <DropdownMenuItem disabled={!canDelete} className="text-destructive-text" onClick={() => onDelete(user)}>
...
function FormField({ id, label, required, description, children }: {
  ...
  return (
    <div className="space-y-2">
      <Label htmlFor={id}>{label}{required && <span className="ml-1 text-destructive-text" aria-hidden="true">*</span>}</Label>
      {children}
