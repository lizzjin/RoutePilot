    ...
    <div className="grid gap-1.5 sm:grid-cols-[92px_1fr]">
      ...
      <div
        className={cn(
          ...
          highlight && 'font-medium text-destructive-text',
...
const toneClasses: Record<Tone, { dot: string; surface: string; text: string; border: string }> = {
  ...
  rose: {
    ...
    border: 'border-destructive-text/30',
  },
  slate: {
    dot: 'bg-muted',
    surface: 'bg-muted',
  ...
  violet: {
    dot: 'bg-category',
    surface: 'bg-category-soft',
    text: 'text-category-text',
    border: 'border-category-text/30',
  },
}
...
function statusBadgeClass(status: RequestLog['status']): string {
  if (status === 'error') return 'border-destructive-text/30 bg-destructive-soft text-destructive-text'
  if (status === 'timeout') return 'border-warning-text/30 bg-warning-soft text-warning-text'
  return 'border-success-text/30 bg-success-soft text-success-text'
}
...
function SignalTile({
  ...
  return (
    <div className={cn('rounded-lg border p-3', classes.border, classes.surface)}>
      <div className={cn('flex items-center gap-2 text-xs font-medium', classes.text)}>
  ...
  return (
    <div className="min-w-0 rounded-lg border bg-muted/20">
      <div className="flex items-center justify-between border-b px-3 py-2">
        ...
        <Badge variant="outline" className="font-mono text-xs">redacted</Badge>
      </div>
      <pre tabIndex={0} className="max-h-56 overflow-auto p-3 text-xs leading-5 text-muted-foreground">
        {codeSnippet(value)}
      </pre>
    </div>
  )
    ...
    <div className="space-y-6">
      ...
      {log.errorMessage && (
        <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-4">
          <p className="text-xs font-medium text-destructive-text">错误信息</p>
          <p className="mt-1 break-words text-sm text-destructive-text">{log.errorMessage}</p>
        </div>
      )}
    </div>
  )
...
function ToolUseTab({ log }: { log: RequestLog }) {
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-lg border border-warning-text/30 bg-warning-soft p-4">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-sm font-semibold text-warning-text">本次请求没有工具级遥测</p>
          ...
          <Badge variant="outline" className="shrink-0 border-warning-text/30 bg-warning-soft text-warning-text">
            能力参考
          </Badge>
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="rounded-lg border bg-muted/20 p-4">
          <div className="flex items-center gap-2">
            <Wrench className="h-4 w-4 text-info-text" />
        ...
        <div className="rounded-lg border bg-muted/20 p-4">
          <div className="flex items-center gap-2">
            <Braces className="h-4 w-4 text-category-text" />
        ...
        <div className="divide-y text-sm">
          {[
            ...
            <div key={event} className="grid gap-2 px-4 py-3 sm:grid-cols-[150px_1fr]">
              <code className="font-mono text-xs text-info-text">{event}</code>
...
export function LogsDrawer({
  ...
  useEffect(() => {
    if (selectedId && window.matchMedia('(max-width: 700px)').matches) closeButtonRef.current?.focus()
  }, [selectedId])
  if (!log) return null
  ...
  return (
    <section className="border bg-card" aria-labelledby="log-detail-title">
      <div>
        {/* Header */}
        <div className="border-b px-5 py-4">
          <div className="flex items-start justify-between gap-4">
            ...
            <Button ref={closeButtonRef} variant="ghost" size="icon" onClick={onClose} aria-label="返回请求列表">
              <X className="h-5 w-5" />
            </Button>
          </div>
        </div>
        ...
        <div className="min-w-0 p-5">
          {detailLoading && (
            <div className="mb-4 flex items-center gap-2 rounded-lg border border-info-text/30 bg-info-soft px-3 py-2 text-xs text-info-text" role="status">
              <Loader2 className="h-3.5 w-3.5 animate-spin" />
              正在从详情接口加载完整请求记录…
            </div>
          )}
          {Boolean(detailError) && !detailLoading && (
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-warning-text/30 bg-warning-soft px-3 py-2 text-xs text-warning-text" role="alert">
