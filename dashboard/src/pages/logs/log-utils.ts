...
export function rowTone(status: RequestStatus): string {
  if (status === 'error') return 'border-l-destructive bg-destructive-soft/25 hover:bg-destructive-soft/50'
  if (status === 'timeout') return 'border-l-warning bg-warning-soft/25 hover:bg-warning-soft/50'
  return 'border-l-transparent'
}

export function providerTone(provider: string): string {
  const key = provider.toLowerCase()
  if (key.includes('mimo')) return 'border-warning-text/30 bg-warning-soft text-warning-text'
  if (key.includes('deepseek')) return 'border-info-text/30 bg-info-soft text-info-text'
  if (key.includes('openai')) return 'border-success-text/30 bg-success-soft text-success-text'
  if (key.includes('anthropic')) return 'border-category-text/30 bg-category-soft text-category-text'
  if (key.includes('gemini')) return 'border-info-text/30 bg-info-soft text-info-text'
  if (key.includes('dashscope')) return 'border-warning-text/30 bg-warning-soft text-warning-text'
  return 'border-border bg-muted text-muted-foreground'
}

export function latencyTone(value: number): string {
  if (value >= 6000) return 'bg-destructive'
  if (value >= 2500) return 'bg-warning'
  return 'bg-success'
}
