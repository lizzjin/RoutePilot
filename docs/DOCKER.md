...
phase-one production profile uses one RoutePilot
...
| `routepilot` | Rust data plane, control API, routing, metrics, and CLI. |
| `dashboard` | Static React UI plus same-origin proxy to backend routes. |
| `routepilot-postgres-18` | Persistent PostgreSQL 18 data. |
| `routepilot-data` | Backend working directory and explicit backup files. |
...
The Compose backend runs as the image's unprivileged `routepilot` user with an
...
- `/data`, backed by the `routepilot-data` named volume, for explicit CLI
...
stored in `routepilot-postgres-18`.
...
`io.routepilot.source-state=clean` in the backend image. For local-only testing
...
```bash
ROUTEPILOT_COMPOSE_ENV_FILE=deploy/docker/routepilot.env.example \
  docker compose --env-file deploy/docker/routepilot.env.example \
    -f "$ROUTEPILOT_COMPOSE_FILE" config --quiet
```
...
ANTHROPIC_AUTH_TOKEN=<same-as-ROUTEPILOT_AUTH_TOKEN>
...
docker compose -f "$ROUTEPILOT_COMPOSE_FILE" logs -f dashboard
docker compose -f "$ROUTEPILOT_COMPOSE_FILE" restart routepilot
scripts/build-container.sh && ROUTEPILOT_LOCAL_BUILD=1 scripts/compose-up.sh
docker compose -f "$ROUTEPILOT_COMPOSE_FILE" down
```

`docker compose -f "$ROUTEPILOT_COMPOSE_FILE" down` preserves named volumes;
adding `-v` deletes PostgreSQL and backend data and is irreversible without a
backup.

## Storage

Unless `.env` explicitly sets `ROUTEPILOT_DATABASE_URL`, Compose constructs it
...
`jsonb` documents in `routepilot_state`:
...
root; enabling `ROUTEPILOT_ENTERPRISE_MODE=1` enforces that boundary.

Compose interpolation does not percent-encode
`ROUTEPILOT_POSTGRES_PASSWORD`. Use a long URL-safe value made from letters,
digits, `_`, and `-`, or explicitly set a complete `ROUTEPILOT_DATABASE_URL`
whose password component is percent-encoded. Keep
`ROUTEPILOT_POSTGRES_PASSWORD` itself as PostgreSQL's raw password. Characters
...
container. It never connects `routepilot` to the temporary database and never
writes to the production database. Completed archives older than
`ROUTEPILOT_BACKUP_RETENTION_DAYS` (14 by default) are pruned only from the
...
```bash
docker compose -f "$ROUTEPILOT_COMPOSE_FILE" exec routepilot \
  routepilot backup export /data/routepilot-backup.json
docker compose -f "$ROUTEPILOT_COMPOSE_FILE" exec routepilot \
  routepilot backup validate /data/routepilot-backup.json
```
...
```bash
docker compose -f "$ROUTEPILOT_COMPOSE_FILE" stop routepilot dashboard
docker compose -f "$ROUTEPILOT_COMPOSE_FILE" run --rm routepilot \
  routepilot backup restore /data/routepilot-backup.json --yes
scripts/compose-up.sh
```
...
```bash
docker compose -f "$ROUTEPILOT_COMPOSE_FILE" exec postgres \
  pg_dump -U routepilot routepilot > routepilot.sql
docker compose -f "$ROUTEPILOT_COMPOSE_FILE" exec -T postgres \
  psql -U routepilot routepilot < routepilot.sql
```

The Compose project has `name: routepilot`; a physical volume backup therefore
uses volume `routepilot_routepilot-postgres-18`. PostgreSQL 18 stores data below
...
is the accepted phase-one topology. It intentionally contains one RoutePilot
instance and no PostgreSQL service. It requires:

- digest-pinned backend and dashboard images;
- an external `routepilot_default` network, shared with approved local inference
...
- a permission-`0600`, short-lived runtime env file rendered outside the
  ...
  `ROUTEPILOT_HEALTHCHECK_API_KEY` for authenticated readiness.
...
`verify-full` and reference `/run/routepilot/database-ca.pem` when an explicit
...
export ROUTEPILOT_IMAGE='registry/routepilot@sha256:<digest>'
export ROUTEPILOT_DASHBOARD_IMAGE='registry/routepilot-dashboard@sha256:<digest>'
export ROUTEPILOT_RUNTIME_ENV_FILE=/run/routepilot/runtime.env
export ROUTEPILOT_CONFIG_FILE=/etc/routepilot/config.toml
export ROUTEPILOT_DATABASE_CA_FILE=/etc/routepilot/database-ca.pem
export ROUTEPILOT_OWNERSHIP_FILE=/etc/routepilot/ownership.toml
...
This profile is not active-active. A second RoutePilot instance remains a later
...
files by default. Override `ROUTEPILOT_LOG_MAX_SIZE` or
`ROUTEPILOT_LOG_MAX_FILES` only after checking host disk capacity.

## Reload And Restart

Compose mounts `.env` read-only at `/config/.env` and sets
`ROUTEPILOT_ENV_FILE=/config/.env`. The dashboard can reload mounted TOML and
env-file-only values for new requests, but process environment values take
precedence over the mounted file.

Restart or recreate `routepilot` for:
...
```bash
scripts/compose-up.sh routepilot
```
...
ROUTEPILOT_TRUSTED_PROXIES=<exact-proxy-ip-or-cidr>
```

`ROUTEPILOT_ALLOWED_ORIGINS` is an admin-write check, not browser CORS. Keep the
...
RoutePilot then walks forwarded hops from right to left and removes only peers
covered by `ROUTEPILOT_TRUSTED_PROXIES`. If another reverse proxy is added in
...
`ROUTEPILOT_ALLOW_INSECURE_PROVIDER_HTTP=1` is set. That override sends Provider
