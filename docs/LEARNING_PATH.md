...
RoutePilot is an authentication, routing, protocol, policy, and evidence
...
A Provider key belongs only in RoutePilot. A RoutePilot API key is what Claude
...
3. Liveness passes but readiness fails: inspect PostgreSQL, migrations, and
   state revisions.
4. HTTP 401/403: verify the RoutePilot key, account state, and policy.
