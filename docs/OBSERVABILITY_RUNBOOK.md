...
[`deploy/observability/prometheus/routepilot-alerts.yml`](../deploy/observability/prometheus/routepilot-alerts.yml).
...
## RoutePilotMetricsTargetDown

1. Request `/livez` directly from the RoutePilot host. If it succeeds, inspect
   Prometheus network reachability, DNS, the scoped metrics key, and whether
   `/metrics` returns 401.
2. If liveness fails, run
   `docker compose -f "${ROUTEPILOT_COMPOSE_FILE:-docker-compose.yml}" ps` and
...
2. Read the safe readiness category and backend logs. Check PostgreSQL DNS,
   ...
   acquisition timeout.
3. Inspect `routepilot_ledger_operation_degraded` and
   `routepilot_ledger_operation_failures_total` by operation.
4. Keep traffic stopped until readiness returns. A Provider test does not repair
   persistence readiness.

`routepilot_gateway_ready` is computed during the authenticated metrics scrape.
...
## RoutePilotDirtyBuildRunning

1. Compare `routepilot_build_info` with the intended release and deployment
   record.
...
## RoutePilotLedgerOperationDegraded
...
3. Check `routepilot_ledger_pending_finalizers` and the request log for
...
`routepilot_database_ready` reports the direct readiness query.
`routepilot_database_pool_utilization_ratio` and the bounded connection-state
...
## RoutePilotDatabasePoolSaturated
...
## RoutePilotPendingFinalizersStuck
...
## RoutePilotExpiredLeasesReconciled
...
## RoutePilotInferenceErrorRateHigh
...
## RoutePilotInferenceP95LatencyHigh
...
2. Check Provider-specific cumulative mean:
   ...
   ```promql
   sum by (provider) (rate(routepilot_message_duration_ms_total[10m]))
   /
   clamp_min(sum by (provider) (rate(routepilot_message_requests_total[10m])), 0.001)
   ```
...
## RoutePilotAdmissionOrStreamPressure
...
4. Inspect `routepilot_local_scheduler_{running,interactive_queued,batch_queued,
   users_queued,estimated_service_ms,oldest_interactive_wait_ms,
   oldest_batch_wait_ms}` and `routepilot_stream_permits_available`. Available
...
## RoutePilotQuotaRejectionsDetected
...
## RoutePilotProviderErrorRateHigh
...
4. Compare completed traffic with `routepilot_provider_available` and
   `routepilot_provider_cooldown`. Availability means a usable credential and no
