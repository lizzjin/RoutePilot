...
This guide covers a running RoutePilot instance. Docker-specific storage and
...
`ROUTEPILOT_FORCE_BUILD=1 scripts/start.sh` to force a rebuild.
...
4. Roll back immediately by setting `ROUTEPILOT_SMART_ROUTING_MODE=shadow` or
...
`routepilot_routing_decisions_total` and
`routepilot_routing_shadow_disagreements_total`. Durable evidence is stored in
`routepilot_routing_decisions` and is also included as `routingDecision` in
...
that returned text instead. These are request-level observations; RoutePilot
...
The row's `statusCode` is RoutePilot's effective client-facing HTTP status for
...
when RoutePilot receives a trusted Provider-reported amount or applies an exact,
...
adapter exposed Provider usage, while `local-estimate` means RoutePilot used its
...
A request is chargeable only after RoutePilot actually starts an upstream
...
`routepilot_inference_rejections_total` metric by pre-ledger phase and safe error
...
explicit preview/apply operation; RoutePilot does not silently evict rows from
an in-document ring.
...
RoutePilot never intentionally persists prompt, response, tool name, tool
...
logging integration, crash dump, or host outside RoutePilot's ledger.
...
`ROUTEPILOT_USER_USAGE_RETENTION_DAYS`, and
`ROUTEPILOT_AUDIT_RETENTION_DAYS`. The periods must remain ordered
...
Set `ROUTEPILOT_RETENTION_LEGAL_HOLD=1` during an approved hold and restart. A
...
console session, `X-RoutePilot-CSRF: 1`, and the normal same-origin check.
...
zero-residue test, use an isolated RoutePilot database, export the result, and
...
- `routepilot_route_{requests,successes,failures,duration_ms}_total`
- `routepilot_inference_rejections_total`
- `routepilot_message_{requests,successes,failures,duration_ms}_total`
- `routepilot_message_{input,output,cache_write,cache_read}_tokens_total`
- `routepilot_message_cost_estimate_usd_total`
- `routepilot_message_latency_ms` (global histogram)
- `routepilot_ledger_operation_failures_total`
- `routepilot_ledger_operation_degraded`
- `routepilot_ledger_reconciled_{requests,attempts}_total`
- `routepilot_ledger_pending_finalizers`
- `routepilot_gateway_ready`
- `routepilot_database_ready`
- `routepilot_database_pool_connections`, `routepilot_database_pool_max_connections`, and `routepilot_database_pool_utilization_ratio`
- `routepilot_provider_available` and `routepilot_provider_cooldown`
- `routepilot_local_scheduler_{running,interactive_queued,batch_queued,users_queued,estimated_service_ms,oldest_interactive_wait_ms,oldest_batch_wait_ms}`
...
operation proves recovery. `routepilot_ledger_pending_finalizers` should normally
return to zero promptly and is drained for up to
`ROUTEPILOT_FINALIZATION_DRAIN_TIMEOUT_SECONDS` during graceful shutdown.
...
RoutePilot targets single-host personal and small-team traffic. Upstream
...
- size `ROUTEPILOT_MAX_CONCURRENT_STREAMS` for simultaneously open bodies, not
...
`ROUTEPILOT_MAX_CONCURRENT_STREAMS` bounds established or establishing streaming
requests independently of the normal Axum request service future. When unset,
it inherits the effective `ROUTEPILOT_MAX_CONCURRENT_REQUESTS`. The stream permit
...
If no permit is immediately available, RoutePilot returns HTTP 429
...
```bash
routepilot backup export /secure/routepilot-backup.json
routepilot backup validate /secure/routepilot-backup.json
routepilot backup restore /secure/routepilot-backup.json --yes
```
...
is available only over HTTP, `ROUTEPILOT_ALLOW_INSECURE_PROVIDER_HTTP=1` is the
...
`ROUTEPILOT_LEDGER_LEASE_TTL_SECS`. The guard remains alive through the complete
stream body, including slow delivery. At startup and every
`ROUTEPILOT_LEDGER_RECONCILE_INTERVAL_SECS`, RoutePilot terminalizes expired
...
| 400 before upstream | Model/messages/Tool Use guardrails; `max_tokens` is required, positive, and capped by `ROUTEPILOT_MAX_OUTPUT_TOKENS`. |
...
| SSE ends at `ROUTEPILOT_HTTP_REQUEST_TIMEOUT_SECS` | Expected total upstream lifecycle limit. Check the configured total and idle timeouts before increasing either; post-header failure is reported in the event stream. |
...
| CPA request shows excessive attempts or latency | Verify CPA `request-retry: 0` and a bounded `max-retry-credentials`; RoutePilot already owns retry/fallback and records each RoutePilot attempt. |
| CPA returns 401 | Check the CPA client key in `CPA_CODEX_API_KEY`/`CPA_CLAUDE_API_KEY`, not the upstream OAuth token; then inspect CPA auth state without copying auth files into RoutePilot. |
...
| Dashboard cross-origin failure | Use a same-origin reverse proxy; `ROUTEPILOT_ALLOWED_ORIGINS` is not a CORS switch. |
| `config validate` rejects enterprise mode | Set a valid `ROUTEPILOT_DATABASE_URL`, use `ROUTEPILOT_DATABASE_TLS_MODE=verify-full`, and fix any reported pool, lease, proxy, or origin syntax before restarting. Validation errors intentionally prevent the server from binding. |
