...
This runbook covers the phase-one single-RoutePilot deployment. It is designed
...
`pg_upgrade` workflow. RoutePilot uses logical dump/restore because it also
...
- restricted RoutePilot and migration identities;
...
1. Record the source RoutePilot revision, image digest, PostgreSQL version,
...
2. Create a new schema-v2 backup:
   ...
   source and target versions; it never connects RoutePilot to the target and
...
3. Restore the dump into an isolated target database with the target
   ...
   `--no-privileges`.
4. Start the candidate RoutePilot revision against only that isolated target.
   Let embedded SQLx migrations finish before sending requests.
5. Compare source and target counts for `_sqlx_migrations`,
   `routepilot_state`, `routepilot_gateway_requests`,
   `routepilot_provider_attempts`, budget/evidence tables, and incomplete
...
The current phase has one RoutePilot instance, so cutover requires a maintenance
window:
...
3. Stop only RoutePilot and the dashboard. Keep the source PostgreSQL database
...
6. Render short-lived runtime credentials from the secret manager. The
   ...
   `/run/routepilot/database-ca.pem`.
...
Keep the source database and previous RoutePilot image immutable through the
rollback window. If validation fails:

1. stop client traffic;
2. stop the candidate RoutePilot instance;
3. restore the previous secret reference and database endpoint;
4. start the previous digest-pinned RoutePilot image against the untouched
   source database;
