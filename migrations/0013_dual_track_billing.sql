...
ALTER TABLE routepilot_gateway_requests
    ...
    ADD CONSTRAINT routepilot_gateway_requests_reconciled_cost_check CHECK (
...
ALTER TABLE routepilot_provider_attempts
    ...
    ADD COLUMN billable_cost_microunits BIGINT,
    ADD COLUMN pricing_evidence JSONB,
    ADD CONSTRAINT routepilot_provider_attempts_reconciled_cost_check CHECK (
        (actual_cost_microunits IS NULL OR actual_cost_microunits >= 0)
        AND (billable_cost_microunits IS NULL OR billable_cost_microunits >= 0)
        AND (billable_cost_microunits IS NULL OR pricing_evidence IS NOT NULL)
    );

CREATE INDEX routepilot_gateway_requests_billable_created_idx
    ON routepilot_gateway_requests (created_at DESC, billable_cost_microunits)
    WHERE billable_cost_microunits IS NOT NULL;

CREATE INDEX routepilot_provider_attempts_unreconciled_idx
