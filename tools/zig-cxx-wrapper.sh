...
if [[ -n "${ZIG_BIN:-}" ]]; then
  if ! zig_bin="$(command -v -- "$ZIG_BIN" 2>/dev/null)"; then
    printf 'routepilot: ZIG_BIN does not point to an executable Zig compiler: %s\n' "$ZIG_BIN" >&2
    exit 127
  fi
  ...
  printf 'routepilot: Zig compiler not found; install zig or set ZIG_BIN to its executable path\n' >&2
  exit 127
fi
